...
const {
  user
} = require('express-gateway/lib/services/');
...
require("body-parser").urlencoded({
  ...
  module.exports = function (gatewayExpressApp) {
    ...
    gatewayExpressApp.use(bodyParser.urlencoded({
      limit: '50mb',
      extended: true
    }));
  ...
  gatewayExpressApp.post('/register', async (req, res, next) => { // code=10 for pdv where he has /api/completed-register
    try {
     ...
     const myUserJwt = await jwt.sign({username:username,password:password},  `${port.JWT_SECRET}`, {
      ...
      const creteProfile = async (myUser) => {
        try {
          return await axios.post(`http://localhost:${port.HTTP_PORT_API_MANAGEMENT}/api_management/user_management/profile`, {
            id_user: myUser.id,
    ...
    gatewayExpressApp.post('/company', async (req, res, next) => { // code=10 for pdv where he has /api/completed-register
      try {
        console.log("*********************************", req.body)
        console.log("/company")
        ...
        const profile = {
          ...
          phone: userbody.phone,
          typeId: userbody.typeId,
          commercial_register: userbody.commercial_register,
          city: userbody.city,
          zip_code: userbody.zip_code,
          adresse: userbody.adresse,
          id_commercial: userbody.id_commercial,
          activity: userbody.activity,
          createdBy: req.body.createdBy

        }


  const confirm_uri = `http://localhost:${port.HTTP_PORT}/registration_confirm?username=` + username + "&" + "confirm_token=" + myUserJwt;
  console.log("confirm_uri",confirm_uri)
  //here je vais envoyer un mail
        ...
        //     }
        //   }
        //   console.log(creteProfile(profile))
    ...
    gatewayExpressApp.post('/registration_confirm', async (req, res, next) => { // code=10 for pdv where he has /api/completed-register
      try {
        ...
        let decoded;
        try {
          decoded = await jwt.verify(confirm_token, '54v3WJGBcFPh3TFgZSzovw', {
            algorithms: ['HS256']
          });
        ...
        user_res = await services.user.update(user.id, {
          confirmMail: 'true'
        })
    ...
    //   }
      ...
      const updateprofile = async () => {
        try {
          return await axios.patch(`http://localhost:${port.HTTP_PORT_API_MANAGEMENT}/api_management/user_management/company/` + req.params.id, {
    ...
    // /////////////
      ...
      try {
        return await axios.post(`http://localhost:${port.HTTP_PORT_API_MANAGEMENT}/api_management/user_management/company/profile-by-company`, { ///profile-by-company
          idOwner: idOwner,
        ...
        return res.status(201).json({
          etat: "Success",
          message: "We have sent an email to " + agentUser.email + " to set a new password"
        });
    ...
    gatewayExpressApp.patch('/activate/:id', verifyTokenSuperAdminOrAdmin, async (req, res, next) => {
      const {
        code
      } = req.body // code = 10 desactive , 11 active
      if (!code) {
        return res.status(200).json({
          error: "Code can not be empty (set 10 to desactivate or 11 to activate a user"
        });
    ...
    gatewayExpressApp.post('/admin_register', verifyTokenSuperAdmin, async (req, res, next) => {
      ...
      const getType = async (code) => {
        ...
        try {
          return await axios.post(`http://localhost:${port.HTTP_PORT_API_MANAGEMENT}/api_management/user_management/profile/agent`, {
            id_user: agentUser.id,
            first_name: agentUser.firstname,
            last_name: agentUser.lastname,
            phone: agentUser.phone,
        ...
        return res.status(201).json({
          etat: "Success",
          message: "Admin has been successfuly created, we have sent an email to " + email + " to set a new password"
        });
    ...
    async function verifyTokenUser(req, res, next) {
      ...
      if (bearerHeader) {
        ...
        try {
          ...
          let decoded;
          try {
            decoded = await jwt.verify(token, '54v3WJGBcFPh3TFgZSzovw', {
              algorithms: ['HS256']
            });
          ...
          if (myCredOauth.scopes) {
            if (myCredOauth.scopes[0] == endpointScopes) {
              ...
              let errorObject = {
                message: 'Unauthorized Token. cannot'
              }
  ...
  console.log("eeee",password)
    ...
    if(myUser == false){
      ...
      const getToken = async (username, password, client_id, client_secret) => {
        try {
          return await axios.post(`http://localhost:${port.HTTP_PORT}/oauth2/token`,{
            grant_type: "password",
            username: username,
            password: password,
            client_id: client_id,
      ...
      ///////////
          ...
          const getProfile = async (id) => {
            try {
              return await axios.get(`http://localhost:${port.HTTP_PORT_API_MANAGEMENT}/api_management/user_management/profile/by_userId/`+id)
            } catch (error) {
              console.error(error)
            }
          }
...
return res.status(400).json("error",error);
  ...
  gatewayExpressApp.post('/forgot', async (req, res, next) => { //get email from user change to email
    ...
    const myUserJwt = await jwt.sign({username:username},  `${port.JWT_SECRET}`, {
      issuer: 'express-gateway',
      audience: 'something',
      expiresIn: 18000,
      subject: '3pXQjeklS3cFf8OCJw9B22',
    ...
    const confirm_uri = `http://localhost:${port.HTTP_PORT}/reset?username=` + username + "&" + "token=" + myUserJwt;
    console.log("confirm_uri",confirm_uri)
    //here je vais envoyer un mail
    ...
    return res.status(201).json({etat: "Success",message:"Check your email : " + user.email});
      ...
      const myUserJwt = await jwt.sign({
        username: username
      }, '54v3WJGBcFPh3TFgZSzovw', {
        issuer: 'express-gateway',
        audience: 'something',
        expiresIn: 18000,
        subject: '3pXQjeklS3cFf8OCJw9B22',
    ...
    gatewayExpressApp.post('/api/test_password', async (req, res, done) => { // still 
      ...
      const createJWT = await tokenService.createJWT({
        consumerId: req.body.consumerId,
        scopes: req.body.scopes
      })
      const saveJWT = await tokenService.save({
        consumerId: req.body.consumerId,
        scopes: req.body.scopes
      }, {
      ...
      const token = await tokenService.findOrSave(tokenCriteria, {
