...
module.exports = function (gatewayExpressApp) {
  ...
  gatewayExpressApp.post('/register', async (req, res, next) => { // code=10 for pdv where he has /api/completed-register
    try {
      ...
      myProfile = await services.application.insert({
        ...
        // redirectUri: 'http://localhost:8080/api/login_callback/'
  ...
  gatewayExpressApp.post('/admin-register', async (req, res, next) => {
    try {
      ...
      return res.status(201).json({ message: "Admin has been successfuly created : " + myUser.email +" confirm_uri : "+confirm_uri});
  ...
  gatewayExpressApp.get('/api/login_callback', async (req, res, next) => { // code=20 for agent created by admin
    console.log("*********************************", req.body)
...
// res.redirect(login_uri)
      return res.status(200).json({redirect_link : login_uri});
// res.redirectUri(encode_uri_component() for redirect uri)
